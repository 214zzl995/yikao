...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        WEBSOCKET_STOP(state) {
            state.websock.close() // 关闭websocket
            state.websock.onclose() // 关闭websocket
        },
        EXAMINATION_INIT(state, url) {
            state.examination = new WebSocket(url);
            state.examination.onopen = function () {
                console.log("考试线路连接成功！");
            }
            state.examination.onmessage = function (callBack) {
                //console.log("ws接收！");
            ...
            state.examination.onerror = function (e) { //错误
                console.log("ws错误!");
                console.log(e);
            }
            state.examination.onclose = function (e) { //关闭
                console.log("ws关闭！");
                console.log(e);
            }
            state.examination.binaryType = "arraybuffer";
            //发送心跳包
            setInterval(function () {
                console.log("ws发送心跳！");
                var heart = {
                ...
                state.examination.send(makeByte(heart));
            }, 30000)
        },
        EXAMINATION_SEND(state, p) {
            state.examination.send(JSON.stringify(p));
        },
    },
    actions: {
        WEBSOCKET_INIT({
        ...
        WEBSOCKET_STOP({
            ...
            p.type = 2;
            commit('WEBSOCKET_STOP', p)
        },
        EXAMINATION_INIT({
            commit
        }, url) {
            commit('EXAMINATION_INIT', url)
        },
        EXAMINATION_SEND({
            commit
        }, p) {
            p.type = 3;
            commit('EXAMINATION_SEND', p)
        },
    }
})
